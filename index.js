...
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, audioRemind, audioEnd, bombNode, newAudio, soundToggle, bombActivate, bombStop, bombExplode, show, adjust, toggle, reset, blink, count, run, resize;
start = null;
isBlink = false;
isLight = true;
isRun = false;
...
bombActivate = function(){
  bombNode == null && (bombNode = $('#bomb'));
  bombNode.removeClass('exploded');
  if (delay > 0) {
    return bombNode.addClass('active');
  }
};
bombStop = function(){
  bombNode == null && (bombNode = $('#bomb'));
  return bombNode.removeClass('active exploded');
};
bombExplode = function(){
  bombNode == null && (bombNode = $('#bomb'));
  bombNode.removeClass('active');
  return bombNode.addClass('exploded');
};
show = function(){
  isShow = !isShow;
